							...
							<div class="tab-content" id="nav-tabContent">
							  	<div class="tab-pane fade active show" id="nav-deactivate" role="tabpanel" aria-labelledby="nav-deactivate-tab">
							  		<div class="acc-setting">
										<h3>채용 공고</h3>
										<form action="${contextPath}/job/jobUpload" method="post" onsubmit="return check(this)">
											<input type="hidden" name="pSeq" value="${pdto.pSeq }" />
											<div class="cp-field">
												<span style="float: left; font-size: 50px; font-weight: bold;">${pdto.pName }</span>
												<img style="float: right; border: 1px solid black;" height="100px" width="150px" src="${contextPath}/resources/upload/${pdto.pPic}"	alt="">
											</div>
									
											<div class="cp-field">
												<h5>채용 공고 제목</h5>
												<div class="cpp-fiel">
													<input type="text" name="jpostName" placeholder="name">
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name = "etypeSeq" id="etypeSeq">
														<c:forEach var="elist" items="${elist}">
															<option value = "${elist.etypeSeq }">${elist.etypeName }</option>
														</c:forEach>
													</select>
												</div>
											</div>
											...
											<div class="cp-field">
												<h5>공고 내용</h5>
												<div class="cpp-fiel">
													<textarea id="jexplain" name="jpostExplain" placeholder="공고 내용"></textarea>
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name="wtypeSeq" id="iSeq">
														<c:forEach var="wlist" items="${wlist }">
															<option value="${wlist.wtypeSeq }"> ${wlist.wtypeName } </option>
														</c:forEach>
													</select>
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name="iSeq" id="iSeq">
														<c:forEach var="ilist" items="${ilist }">
															<option value="${ilist.iSeq }"> ${ilist.iName } </option>
														</c:forEach>
													</select>
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name="occSeq" id="iSeq">
														<c:forEach var="olist" items="${olist }">
															<option value="${olist.occSeq }"> ${olist.occName } </option>
														</c:forEach>
													</select>
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name="posSeq" id="iSeq">
														<c:forEach var="plist" items="${plist }">
															<option value="${plist.posSeq }"> ${plist.posName } </option>
														</c:forEach>
													</select>
												</div>
											</div>
												...
												<div class="cpp-fiel">
													<select name="jrankSeq" id="iSeq">
														<c:forEach var="jlist" items="${jlist }">
															<option value="${jlist.jrankSeq }"> ${jlist.jrankName } </option>
														</c:forEach>
													</select>
												</div>
											</div>
											...
											<div class="cp-field">
												<h5>채용 인원</h5>
												<div class="cpp-fiel">
													<input type="number" name="jpostCnt" placeholder="0">
												</div>
											</div>
											
											<div class="save-stngs pd3">
												<ul>
													<li><button type="submit">채용 공고 생성</button></li>
